...
本模板由作者在毕业论文排版实践中整理，并结合 Codex 辅助完成结构抽取、样式调整、README 编写和示例编译检查；最终内容由作者核对和维护。
...
公开发布、二次分发或用于正式论文提交前，建议再次检查：

- 不提交 `main.pdf`、`main.docx`、`*.aux`、`*.bbl`、`*.log`、`*.toc` 等临时或生成文件；本仓库仅保留 `example.pdf` 作为示例。
- 不公开个人论文正文、实验数据、未授权图片、签名页扫描件等敏感或版权不明内容。
- 检查 `main.tex`、Word 模板、示例 PDF 中是否仍包含个人姓名、学号、导师、论文题目等信息。
